...
public:
    ...
    int Alloc(Operator<float> *x){
        ...
        out = new ConvolutionLayer2D<float>(out, 1, 512, 3, 3, 2, 2, 1, 1, "D_Conv1");
        out = new BatchNormalizeLayer<float>(out, TRUE, "D_BN1");
        out = new LRelu<float>(out, 0.2, "D_Relu1");

        // input 14x14 output 7x7
        out = new ConvolutionLayer2D<float>(out, 512, 256, 3, 3, 2, 2, 1, 1, "D_Conv2");
        out = new BatchNormalizeLayer<float>(out, TRUE, "D_BN2");
        out = new LRelu<float>(out, 0.2,  "D_Relu2");

        // input 7x7 output 4x4
        out = new ConvolutionLayer2D<float>(out, 256, 128, 3, 3, 2, 2, 1, 1, "D_Conv2");
        out = new BatchNormalizeLayer<float>(out, TRUE, "D_BN2");
        out = new LRelu<float>(out, 0.2,  "D_Relu2");

        out = new ReShape<float>(out, 1, 1, 128 * 4 * 4, "D_ReShape3");
        out = new Linear<float>(out, 128 * 4 * 4, 1);
